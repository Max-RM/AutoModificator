...
rem Prefer triplet compiler for x64, otherwise use generic clang++ with --target, otherwise g++
if exist "%BIN%\x86_64-w64-mingw32-clang++.exe" (
	set "CC=%BIN%\x86_64-w64-mingw32-clang++.exe"
) else if exist "%BIN%\clang++.exe" (
	set "CC=%BIN%\clang++.exe"
	set "CC_NEEDS_TARGET=1"
) else if exist "%BIN%\x86_64-w64-mingw32-g++.exe" (
	set "CC=%BIN%\x86_64-w64-mingw32-g++.exe"
) 

if not defined CC (
	echo ERROR: No suitable C++ compiler found in %BIN% ^(x86_64-w64-mingw32-clang++, clang++, or x86_64-w64-mingw32-g++^)
	exit /b 1
)
...
if not exist "%RC%" (
	...
	if not exist "%RC%" (
		set "RC=%BIN%\x86_64-w64-mingw32-windres.exe"
	)
	if not exist "%RC%" (
...
set OUT=Auto_Modificator_llvm_019_x64.exe
...
set "TARGET_FLAG="
if "%CC_NEEDS_TARGET%"=="1" set "TARGET_FLAG=--target=x86_64-w64-windows-gnu"
