...
rem Prefer triplet compiler for x86, otherwise use generic clang++ with --target, otherwise g++
if exist "%BIN%\i686-w64-mingw32-clang++.exe" (
	set "CC=%BIN%\i686-w64-mingw32-clang++.exe"
) else if exist "%BIN%\clang++.exe" (
	set "CC=%BIN%\clang++.exe"
	set "CC_NEEDS_TARGET=1"
) else if exist "%BIN%\i686-w64-mingw32-g++.exe" (
	set "CC=%BIN%\i686-w64-mingw32-g++.exe"
)

if not defined CC (
	echo ERROR: No suitable C++ compiler found in %BIN% ^(i686-w64-mingw32-clang++, clang++, or i686-w64-mingw32-g++^)
	exit /b 1
)
...
if not exist "%RC%" (
	...
	if not exist "%RC%" (
		set "RC=%BIN%\i686-w64-mingw32-windres.exe"
	)
	if not exist "%RC%" (
...
set "TARGET_FLAG="
if "%CC_NEEDS_TARGET%"=="1" set "TARGET_FLAG=--target=i686-w64-windows-gnu"
